...
class TestDataverseBatch:
    ...
    def test_initialization(self, mock_setup_logging, mock_client_class, sample_config):
        ...
        assert call_args[0][0] == sample_config["dynamics_url"]
        assert call_args[0][1] == sample_config["client_id"]
        assert call_args[0][2] == sample_config["client_secret"]
        assert call_args[0][3] == sample_config["tenant_id"]
    ...
    @patch("dataverse_batch.core.DataverseClient")
    @patch("dataverse_batch.core.setup_logging")
    def test_initialization_with_custom_logging(
        self, mock_setup_logging, mock_client_class, sample_config
    ):
        ...
        dataverse_batch = DataverseBatch(
            **sample_config, log_level="DEBUG", log_file="test.log"
        )
    ...
    def test_create_multiple_sequential(
        self,
        ...
        mock_processor_class.assert_called_once_with(
            mock_client_instance, "accounts", 100
        )
        mock_processor_instance.process_sequential.assert_called_once_with(
            sample_account_data[:2]
        )
    ...
    @patch("dataverse_batch.core.setup_logging")
    @patch("dataverse_batch.core.BatchProcessor")
    def test_create_multiple_parallel(
        self,
        mock_processor_class,
        mock_setup_logging,
        mock_client_class,
        ...
        # Verify calls
        mock_processor_class.assert_called_once_with(
            mock_client_instance, "accounts", 50
        )
        mock_processor_instance.process_parallel.assert_called_once_with(
            sample_account_data[:2], 5
        )

    @patch("dataverse_batch.core.DataverseClient")
    @patch("dataverse_batch.core.setup_logging")
    def test_create_multiple_validation_error(
        self, mock_setup_logging, mock_client_class, sample_config
    ):
        ...
        with pytest.raises(ValueError, match="The 'data' parameter must be a list"):
            dataverse_batch.create_multiple(
                data="not a list", table="accounts"  # Invalid data
            )
        ...
        with pytest.raises(
            ...
            dataverse_batch.create_multiple(
                data=[{"name": "Test"}], table=""  # Invalid table name
            )

    @patch("dataverse_batch.core.DataverseClient")
    @patch("dataverse_batch.core.setup_logging")
    def test_get_batch_recommendations(
        self, mock_setup_logging, mock_client_class, sample_config
    ):
        ...
        assert recommendations["batch_size"] == 500
