...
class TestIntegration:
    ...
    def test_end_to_end_success(
        self, mock_setup_logging, mock_client_class, sample_config, sample_account_data
    ):
        ...
        result = dataverse_batch.create_multiple(
            data=sample_account_data[:2], table="accounts", batch_size=2, parallel=False
        )
    ...
    @patch("dataverse_batch.core.DataverseClient")
    @patch("dataverse_batch.core.setup_logging")
    def test_end_to_end_with_recommendations(
        self, mock_setup_logging, mock_client_class, sample_config, sample_account_data
    ):
        ...
        with patch(
            "dataverse_batch.batch_processor.BatchProcessor.process_parallel"
        ) as mock_process:
