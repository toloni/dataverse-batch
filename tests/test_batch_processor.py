...
class TestBatchProcessor:
    ...
    def test_process_sequential_success(self, mock_sleep, mock_tqdm, processor, sample_account_data):
        """Test successful sequential processing"""
        # Mock client responses
        processor.client.create_records_batch.side_effect = [
            [
                {'record_index': 0, 'status': 'success', 'id': '111', 'data': sample_account_data[0]},
                {'record_index': 1, 'status': 'success', 'id': '222', 'data': sample_account_data[1]}
            ],
            [
                {'record_index': 2, 'status': 'success', 'id': '333', 'data': sample_account_data[2]}
            ]
        ]
    ...
    @patch('dataverse_batch.batch_processor.tqdm')
    @patch('dataverse_batch.batch_processor.time.sleep') 
    def test_process_sequential_with_errors(self, mock_sleep, mock_tqdm, processor, sample_account_data):
        """Test sequential processing with errors"""
        # Mock client responses with errors
        processor.client.create_records_batch.side_effect = [
            [
                {'record_index': 0, 'status': 'success', 'id': '111', 'data': sample_account_data[0]},
                {'record_index': 1, 'status': 'error', 'error': 'Name required', 'data': sample_account_data[1]}
            ],
            [
                {'record_index': 2, 'status': 'success', 'id': '333', 'data': sample_account_data[2]}
            ]
    ...
    @patch('dataverse_batch.batch_processor.tqdm')
    def test_process_parallel_success(self, mock_tqdm, mock_executor, processor, sample_account_data):
    ...
    @patch('dataverse_batch.batch_processor.ThreadPoolExecutor')
    @patch('dataverse_batch.batch_processor.tqdm')
    def test_process_parallel_with_exceptions(self, mock_tqdm, mock_executor, processor, sample_account_data):
    ...
    def test_create_results_dataframe(self, processor, sample_account_data):
        ...
        results = [
            {'record_index': 0, 'status': 'success', 'id': '111', 'data': sample_account_data[0]},
            {'record_index': 1, 'status': 'error', 'error': 'Validation failed', 'data': sample_account_data[1]},
            {'record_index': 2, 'status': 'success', 'id': '333', 'data': sample_account_data[2]}
        ]
        ...
        assert len(result_df) == 3
        assert list(result_df.columns) == ['record_index', 'status', 'id', 'error', 'data']
